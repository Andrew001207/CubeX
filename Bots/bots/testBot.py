...
def create_conv_handler(update, context, questions, process_results=lambda answers: None, return_state=ConversationHandler.END, sr=""):
    ...
    ###################
    if questions and isinstance(questions[0], ConversationHandler):
        tmp = questions.pop(0)
    ...
    entry = []
    if return_state != -1:
        def i_start(update, cotext):
            update.message.reply_text(sr)
            print("Inner start done!!!")
            return 0
        print("Creating custom start")
        entry = [MessageHandler(Filters.all, i_start)]

    #Create actual ConversationHandler
    conv_handler = ConversationHandler(
        entry_points=entry, #Starts automated
        ...
        map_to_parent={ConversationHandler.END: return_state}
    )
