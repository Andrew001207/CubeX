...
class MyDispatcher(object):
    ...
    def add_handler(self, handler, group=DEFAULT_GROUP):
        ...
        from myconversationhandler import MyConversationHandler

        if not isinstance(handler, Handler):
            raise TypeError('handler is not an instance of {0}'.format(Handler.__name__))
        if not isinstance(group, int):
            raise TypeError('group is not int')
        if isinstance(handler, MyConversationHandler) and handler.persistent:
        ...
        if isinstance(handler, MyConversationHandler):
            ...
            print("Started ConvHandler in Dispatcher")
